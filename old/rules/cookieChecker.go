...
var cookieRecordExtensionTime = time.Hour * 24 * 365
var sidHashSize = 32
...
func DeleteOldCookieRecords(req *http.Request, res http.ResponseWriter, keepCookieName string, domain string) {
	...
	for _, cookie := range req.Cookies() {
		if cookie.Name != keepCookieName &&
			...
			// jsCode += fmt.Sprintf("document.cookie=\"%s=; path=/; domain=%s; max-age=0; expires=Thu, 18 Dec 2013 12:00:00 UTC\";", cookie.Name, domain)
			// jsCode += fmt.Sprintf("document.cookie=\"%s=; path=/; domain=.%s; max-age=0; expires=Thu, 18 Dec 2013 12:00:00 UTC;\";", cookie.Name, domain)
		}
	}
	...
	// 	res.Header().Set("Content-Type", "text/html")
	// 	res.WriteHeader(200)
	// 	jsCode = fmt.Sprintf("<script>%s; setTimeout(location.reload, 10000);</script>", jsCode)
	// 	_, _ = res.Write([]byte(jsCode))
	//
	// 	return true
	// }
	// return false
